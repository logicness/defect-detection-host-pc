...
> **v1.5.1**：类别名按当前模型动态映射（新建 `core/class_names.py`），修复
 ...
 **[已入库]/[新]** 前缀并显示数据集/类别/质量/推荐，已入库模型禁用重复添加；
 ...
 自动识别扫描常见目录与 `<LOCAL_ROOT>` 等训练目录（深度 5，跳过无用目录）
...
- **v1.2**：体验与离线能力完善。
 ...
 `<LOCAL_ROOT>` 等训练目录，深度 5，跳过无用目录（实测识别 22 个模型）
...
- **v1.0**：首版。5 页 UI 按设计稿实现；双推理源（TCP+本地模拟）；
