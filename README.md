...
> **v1.5.1（2026-08-13）**：类别名按当前模型动态映射（新建 `core/class_names.py`），修复
> MVIT 6 类 / SteelDefectX 25 类结果被显示成 NEU 6 类类名的问题；修复检测框四角角标 float
> 坐标导致 `paintEvent` TypeError 崩溃的 bug；停止检测清框再次加固（clear + repaint + 延迟
> 50ms/150ms 安全网），防止任何竞态下框残留。
>
> **v1.5（2026-08-13）**：重新连接提示精简为仅右下角运行日志（不再弹窗，连接成功弹窗
> 保留 + 10 秒去重）；停止检测彻底清框（忽略延迟推理结果 + 中止本地后台推理 + 本地图片
> 重显原图）；检测框清晰度增强（黑半透明外框 + 亮红内框双层描边、四角白色角标、
> 左上角「类别 置信度」标签）；选图自动定位当前模型训练数据集目录；模型管理扫描结果
> 移到右下角独立列表。
>
...
- **v1.5（2026-08-13）**：重新连接提示精简 + 停止清框 + 检测框清晰度。
  ...
  - 停止检测彻底清框：`_detection_paused` 标志忽略延迟到达的推理结果 +
    `LocalInferEngine.cancel()` 中止本地后台推理；本地图片模式重新显示原图
  - 检测框清晰度增强：双层描边（黑半透明外框 + 亮红内框）+ 四角白色角标 +
    左上角「类别 置信度」标签
  - 选图自动定位当前模型对应训练数据集目录（`resolve_dataset_image_dir`）；
    模型管理对话框扫描结果移到右下角独立列表
- **v1.4（2026-08-12）**：模型管理界面重构为单页三区。
  - 顶部操作栏 + 左侧树形模型列表（本地库/下位机分组 + 搜索过滤）+
    右侧详情+操作卡片；双 Tab 合并功能不砍，1202 行精简到 940 行
